...
def build_cor_mat(drugs_df, muts_df):
    ...
    muts_df_new = muts_df * 1
    concatinated = pd.concat([drugs_ranked, muts_df_new], axis=1)
    concatinated_corr = concatinated.corr(method="spearman")
    return concatinated_corr[drugs_ranked.columns].loc[muts_df_new.columns]
...
def filtered_drugs(drug_mut_cor, mut_drug_df):
    return mut_drug_df.loc[:, mut_drug_df.columns.isin(drug_mut_cor.columns)]
...
def get_mut_drug_predict_matrix(tcga_genes, tcga_muts, beat_rnaseq_shared, beat_drug):
    mut_predict= get_mut_predict(tcga_genes, tcga_muts, beat_rnaseq_shared)
    prediction = general_utils.convert_predict_to_df(mut_predict, tcga_muts.columns, beat_rnaseq_shared.index)
    general_utils.export_drugs_prediction(prediction.transpose(), "mut_predidtion_4")
    mut_drug_df = build_cor_mat(beat_drug, prediction)
    general_utils.export_drugs_prediction(mut_drug_df.transpose(), "mut_drug_results_4")
...
def task_three_flow():
    ...
    mut_drug_df = get_mut_drug_predict_matrix(tcga_genes, tcga_muts, beat_rnaseq_shared_filterd, drugs_filtered)
    result_to_check = filtered_drugs(mut_drug_df)
    print ("filtered_drugs f-> null_count: ", result_to_check.isna().sum().sum())
    return mut_drug_df, result_to_check

if __name__== "__main__" :
    # mut_drug_df, result_to_check = task_three_flow()
    ...
    # beat_rnaseq_shared_filterd, drugs_filtered = filter_samples(beat_rnaseq, beat_drug)
    # mut_predict_df = data_prep_utils.get_df("mut_predidtion_4").transpose()
    # mut_drug_df = build_cor_mat(drugs_filtered, mut_predict_df)
    # drug_mut_cor = data_prep_utils.gene_exp_log_trans(data_prep_utils.get_df("drug_mut_cor")).transpose()
    # mut_drug_df_filt = filtered_drugs(drug_mut_cor, mut_drug_df)
    # print("mse: ", general_utils.get_mse(mut_drug_df_filt,drug_mut_cor))

    mut_drug_df = data_prep_utils.get_df("task_three_corr_matrix").transpose()
    drug_mut_cor = data_prep_utils.gene_exp_log_trans(data_prep_utils.get_df("drug_mut_cor")).transpose()
    mut_drug_df_filt = filtered_drugs(drug_mut_cor, mut_drug_df)
    print("mse: ", general_utils.get_mse(mut_drug_df_filt,drug_mut_cor))
